		...
		if(gt911_touch_state != 0) {
			*num_of_touch_detect = gt911_touch_state;
			for(uint8_t i = 0; i < *num_of_touch_detect; i++) {
				...
				sta = HAL_I2C_Master_Transmit(&hi2c1, GT911_ADDR, txBuf, 2, 100);
				if(sta != HAL_OK) {
					GT911_ERR("get touchP%d write err[%d]\n", i, sta);
					return GT911_STA_GET_TOUCH_WRITE_ERR;
				}
				sta = HAL_I2C_Master_Receive(&hi2c1, GT911_ADDR, rxBuf, 4, 100);
				if(sta != HAL_OK) {
...
static GT911_STA_t gt911_set_state(uint8_t state)
{
	...
	txBuf[2] = state;
	sta = HAL_I2C_Master_Transmit(&hi2c1, GT911_ADDR, txBuf, 3, 100);
	if(sta != HAL_OK) {
...
static GT911_STA_t gt911_get_state(uint8_t* state)
{
	...
	sta = HAL_I2C_Master_Receive(&hi2c1, GT911_ADDR, state, 1, 100);
	if(sta != HAL_OK) {
...
static GT911_STA_t gt911_set_all_config(uint8_t* buf)
{
	...
	sta = HAL_I2C_Master_Transmit(&hi2c1, GT911_ADDR, buf, 188, 100);
	if(HAL_OK != sta) {
...
static GT911_STA_t gt911_get_all_config(uint8_t* buf)
{
	...
	sta = HAL_I2C_Master_Receive(&hi2c1, GT911_ADDR, buf, 185, 100);
	if(sta != HAL_OK) {
...
static GT911_STA_t gt911_get_firmware_version(void)
{
	...
	sta = HAL_I2C_Master_Receive(&hi2c1, GT911_ADDR, rxBuf, 2, 100);
	if(sta != HAL_OK) {
...
static GT911_STA_t gt911_get_config_version(void)
{
	...
	sta = HAL_I2C_Master_Receive(&hi2c1, GT911_ADDR, rxBuf, 1, 100);
	if(sta != HAL_OK) {
